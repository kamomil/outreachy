...
#v4l2-ctl -d2 -x width=$w1,height=$h1 -v width=$w1,height=$h1,pixelformat=$format --stream-mmap --stream-out-mmap --stream-from jelly-$format-$w1-$h1-to-$w2-$h2.fwht --stream-to out-$w1-$h1-to-$w2-$h2.$format &

	v4l2-ctl -d1 -x width=$w2,height=$h2 -v width=$w2,height=$h2,pixelformat=$format --stream-mmap --stream-out-mmap --stream-from jelly-$format-$w2-$h2-to-$w1-$h1.fwht --stream-to out-$w2-$h2-to-$w1-$h1.$format
...
#    codec 1920 1080 crop=860x540 compose=860x540  GREY
	...
	codec 1000 1010 crop=1000x1010 YUYV
	codec 802 910 crop=802x910 422P
	codec 1002 1010 crop=1002x1010 422P
