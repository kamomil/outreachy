...
int main(int argc, char **argv)
{
        ...
        if (argc < 5) {
                printf("usage: %s dev-file fwht-img raw-output-img FORMAT\n", argv[0]);
                printf("'dev_file' is the name of the vicodec device file, e.g. /dev/video1\n");
                printf("'fwht-img': the name of the encoded, assumed to be 640x480\n");
                printf("'raw-output-img' the name of the output image\n");
                printf("'FORMAT': the format of raw-img, e.g. V4L2_PIX_FMT_ARGB32\n");

                exit(EXIT_FAILURE);
        }
        dev_name = argv[1];
        char* fwht_img = argv[2];
        char* raw_img = argv[3];
        ...
        send_frames(fd, buffers_out, fwht_img);
        recv_frames(fd, buffers_cap, raw_img);
